...
def test_check_for_vertex(graph):
    ...
    assert graph.check_for_vertex(pass_vertex1)
    assert graph.check_for_vertex(pass_vertex2)
...
def test_add_vertex(graph, vertices):
    graph._get_vertices()
    with pytest.raises(AlrExistVertexError):
        graph.add_vertex(3)
    graph.add_vertex(6)
    assert graph._vertices == vertices + [6]
...
def test_reverse_iterator_for_incident_edges(graph):
    graph._get_edges()
    
    iterator = graph.reverse_iterator_for_incident_edges(2)
    assert next(iterator) == (4, 2)
    assert next(iterator) == (2, 5)
    assert iterator.prev() == (4, 2)

def test_const_iterator_for_incident_edges(graph):
    graph._get_edges()
    
    iterator = graph.const_iterator_for_incident_edges(2)
    assert next(iterator) == (1, 2)
    assert iterator.peek_next() == (2, 4)
    assert iterator.current() == (1, 2)
    
def test_const_reverse_iterator_for_incident_edges(graph):
    graph._get_edges()
    
    iterator = graph.const_reverse_iterator_for_incident_edges(2)
    
    assert next(iterator) == (4, 2)
    assert next(iterator) == (2, 5)
    assert iterator.peek_next() == (2, 4)
    assert iterator.current() == (2, 5)
